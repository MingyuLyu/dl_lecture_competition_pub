...
class EVFlowNet(nn.Module):
    def __init__(self, args):
        ...
        self.encoder1 = general_conv2d(in_channels=4, out_channels=_BASE_CHANNELS,
                                       do_batch_norm=not self._args.no_batch_norm)
        self.encoder2 = general_conv2d(in_channels=_BASE_CHANNELS, out_channels=2 * _BASE_CHANNELS,
                                       do_batch_norm=not self._args.no_batch_norm)
        self.encoder3 = general_conv2d(in_channels=2 * _BASE_CHANNELS, out_channels=4 * _BASE_CHANNELS,
                                       do_batch_norm=not self._args.no_batch_norm)
        self.encoder4 = general_conv2d(in_channels=4 * _BASE_CHANNELS, out_channels=8 * _BASE_CHANNELS,
                                       do_batch_norm=not self._args.no_batch_norm)
    ...
    def forward(self, inputs):
        ...
        # print("flow0", flow_dict['flow0'].size())
        # print("flow1", flow_dict['flow1'].size())
        # print("flow2", flow_dict['flow2'].size())
        # print("flow3", flow_dict['flow3'].size())
